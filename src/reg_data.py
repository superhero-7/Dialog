...
dataset_dir = Path('/sharefs/baai-mrnd/yfl/database')
...
class RefCOCOGenerationFineTuneDataset(Dataset):
    def __init__(self, refer=None, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train', task=None):
        super().__init__()
        ...
        if self.args.dialog_sp_training and self.mode == 'train':
            print("Load the bad referring expressions.")
            with open(self.args.bad_res_path, 'r') as fp:
                bad_sents = json.load(fp)
            ref_ids_2_bad_sents = {}
            # self.ref_ids_2_bad_bbox = {}
            for item in bad_sents:
                ref_ids_2_bad_sents[str(item['ref_id'])] = item['sent']
                # self.ref_ids_2_bad_bbox[item['ref_id']] = item['bbox']
        ...
        # For Bootstrap
        if self.args.use_bootstrap and self.mode=='train':
            assert self.args.bootstrap_data is not None, 'Fool! You forget to assgin the bootstrap_data path!'
            with open(self.args.bootstrap_data, 'r') as fn:
                bootstrap_data = json.load(fn)
            print("^_^ Loaded the bootstrap data from {}".format(self.args.bootstrap_data))
            # ref_id --> data
            self.bootstrap_dict = {}
            for d in bootstrap_data:
                if d['ref_id'] not in self.bootstrap_dict:
                    self.bootstrap_dict['ref_id'] = [d['sent']]
                else:
                    self.bootstrap_dict['ref_id'] += d['sent']

        if self.args.use_negative_text_training and self.mode == 'train':
            assert self.args.negative_text_training_data is not None, 'Fool! You forget to assgin negative text training data path!'
            with open(self.args.negative_text_training_data) as fn:
                negative_text_training_data = json.load(fn)
            ref_ids_2_negative_sents = {}
            # self.ref_ids_2_bad_bbox = {}
            negative_ref_ids = set()
            for item in negative_text_training_data:
                negative_ref_ids.add(item['ref_id'])
                if item['ref_id'] not in ref_ids_2_negative_sents:
                    ref_ids_2_negative_sents[item['ref_id']] = [item['sent']]
                else:
                    ref_ids_2_negative_sents[item['ref_id']] += item['sent']
            negative_ref_ids = list(negative_ref_ids)
        ...
        for ref_id in ref_ids:
            ...
            if self.mode == 'train':
                if self.args.dialog_sp_training:
                    if str(ref_id) not in ref_ids_2_bad_sents:
                        continue
                    for sent, sent_id in zip(ref["sentences"], ref["sent_ids"]):
                        ...
                        data.append(
                            {
                                "caption": caption,
                                "bad_sent": ref_ids_2_bad_sents[str(ref_id)],
                                "sent_id": sent_id,
                                "image_id": image_id,
                                "refBox": refBox,
                                "ref_id": ref_id,
                ...
                elif self.args.use_negative_text_training:
                    for sent, sent_id in zip(ref["sentences"], ref["sent_ids"]):
                        caption = sent["raw"]
                        if ref_id not in ref_ids_2_negative_sents:
                            pesudo_ref_id = random.choice(negative_ref_ids)
                            data.append(
                                {
                                    "caption": caption,
                                    "negative_sent": random.choice(ref_ids_2_negative_sents[pesudo_ref_id]),
                                    "sent_id": sent_id,
                                    "image_id": image_id,
                                    "refBox": refBox,
                                    "ref_id": ref_id,
                            ...
                            data.append(
                                {
                                    "caption": caption,
                                    "negative_sent": random.choice(ref_ids_2_negative_sents[ref_id]),
                                    "sent_id": sent_id,
                                    "image_id": image_id,
                                    "refBox": refBox,
                                    "ref_id": ref_id,
                    ...
                    if self.args.use_bootstrap and ref_id in self.bootstrap_dict and self.mode=='train':
                        for bootsrap_sent in self.bootstrap_dict[ref_id]:
                            data.append(
                             {
                                "caption": bootsrap_sent,
                                "sent_id": -1,
                                "image_id": image_id,
                                "refBox": refBox,
                                "ref_id": ref_id,
        ...
        self.source_to_h5 = {
            ...
            'train_ofa_target': refcocog_feature_dir.joinpath('train_ofa_target.h5'),
    ...
    def __getitem__(self, idx):
        ...
        if self.args.use_vision:
            ...
            # 这块是加入视觉特征的，感觉很不合理，先去掉吧... 非常合理啊！就是得把视觉特征加进来！
            # if self.args.dialog_sp_training and self.mode == 'train':
            ...
            # 这块也是为了加视觉特征，而且ofa target的特征现在没有抽
            # if self.args.dialog_sp_training and self.mode=='train':
            #     ofa_target_boxes = f_ofa_target[f'{ref_id}/boxes'][:]
            # shuffle box order
            ...
            #     feats_ofa_target = f_ofa_target[f'{ref_id}/features'][:]
        ...
        if self.args.dialog_sp_training and self.mode == 'train':
            ...
            input_text = f'{prefix} {visual_token} {bad_sent} {unlocated} {refine}'
        else:
        ...
        if self.mode == 'train':
            ...
            if self.args.use_negative_text_training:
                negative_sent = datum['negative_sent']
                negative_sent_ids = self.tokenizer.encode(negative_sent, max_length=self.args.max_text_length, truncation=True)
                out_dict['negative_sent'] = negative_sent
                out_dict['negative_sent_ids'] = torch.LongTensor(negative_sent_ids)
                out_dict['negative_sent_length'] = len(negative_sent_ids)
        else:
    ...
    def collate_fn(self, batch):
        ...
        if self.mode == 'train':
            ...
            target_texts = []
            if self.args.use_negative_text_training:
                N_W_L = max(entry['negative_sent_length'] for entry in batch)
                negative_sent_ids = torch.ones(B, N_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
                negative_sents = []
        else:
            sents = []
        ref_ids = []
        ...
        for i, entry in enumerate(batch):
            ...
            if self.mode == 'train':
                ...
                if self.args.use_negative_text_training:
                    negative_sents.append(entry['negative_sent'])
                    negative_sent_ids[i, :entry['negative_sent_length']] = entry['negative_sent_ids']
            else:
        ...
        if self.mode == 'train':
            ...
            if self.args.use_negative_text_training:
                negative_sent_ids[negative_sent_ids == self.tokenizer.pad_token_id] = -100
                batch_entry['negative_sent_ids'] = negative_sent_ids
                batch_entry['negative_sents'] = negative_sents
        else:
