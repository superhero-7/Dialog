...
class VLT5REG(VLT5):
    ...
    def dialog_train_step(self, batch, rewarder, dialog_round=1):
        ...
        for i in range(dialog_round):
            ...
            # 无法保证每个batch中，各样本结束的轮次都是一样的昂！先都拼一个"unlocated"进去试试吧
    ...
    def test_step(self, batch, dialog_training, rewarder, dialog_round=2, last_round=False, threshold=0.5, **kwargs):
        device = next(self.parameters()).device
        vis_feats = batch['vis_feats'].to(device)
        input_ids = batch['input_ids'].to(device)
        vis_pos = batch['boxes'].to(device)
        ...
        if dialog_training:
            ...
            for dialog_round_idx in range(dialog_round):
                ...
                unlocated = "unlocated"
                refine = "refine it:"
                input_text = f'{prefix} {visual_token} {output_sents[0]} {unlocated} {refine}'
    ...
    def new_test_step(self, batch, rewarder, refine_model=None, dialog_round=1, last_round=False, threshold=0.5, detector=None, **kwargs):
        '''
        该函数的功能是最初的refine的那种想法：就是由REC来判定第一轮是否正确，如果正确则不继续；否则会condition on第一轮的结果；
        '''
        ...
        for dialog_round_idx in range(dialog_round):
            ...
            if dialog_round_idx == 0:
                ...
                sample_dict['sents'] = output_sents
                sample_rewards, sample_rewards_mask, det_result = rewarder.compute_score(sample_dict)
