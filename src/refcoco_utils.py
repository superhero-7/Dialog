...
class REFER:
	...
	def __init__(self, dataset='refcoco', splitBy='unc', img_dir=None, ref_dir=None, verbose=True):
		...
		# self.ROOT_DIR = coco_img_dir

		if img_dir is None:
			#img_dir = Path(__file__).resolve().parents[2].joinpath('datasets/COCO/images')
		...
		if dataset in ['refcoco', 'refcoco+', 'refcocog']:
			...
			self.IMAGE_DIR = img_dir.joinpath('train2014')
			if self.verbose:
				print(self.IMAGE_DIR)
		elif dataset == 'refclef':
			# self.IMAGE_DIR = osp.join(data_root, 'images/saiapr_tc-12')
			self.IMAGE_DIR = img_dir.joinpath('saiapr_tc-12')
		else:
		...
		ref_file = self.DATA_DIR.joinpath('refs('+splitBy+').p')
		self.data = {}
		...
		instances_file = self.DATA_DIR.joinpath('instances.json')
